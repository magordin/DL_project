...
#BSUB -W 24:00
#BSUB -o /work3/s252608/DL_project/logs/models_train_%J.out
#BSUB -e /work3/s252608/DL_project/logs/models_train_%J.err
  ...
  for VARIANT in ${REPR_VARIANTS}; do
    ...
    for MODEL_TYPE in ${MODEL_TYPES}; do
      for SEED in ${SEEDS}; do
        ...
        if [[ -f "${MODEL_FILE}" && -f "${HISTORY_FILE}" && -f "${SPLIT_FILE}" ]]; then
          echo "Skipping existing model: ${MODEL_FILE}"
        else
